...
const AdminPanel: React.FC<AdminPanelProps> = ({ onBackToDashboard }) => {
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Template SIGAP");
  ...
  const handleFileProcess = async (file: File) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (parsedData.length === 0) throw new Error('File Excel terdeteksi kosong.');
      ...
      } catch (err: any) { setUploadError(`Gagal mem-parsing berkas Excel: ${err.message}`); } 
  ...
  const handleSaveManualPelanggaran = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccessManual('Sukses menambahkan aturan pelanggaran baru!');
      setJenisManual('');
  ...
  const handleAddGuruManual = async (e: FormEvent) => {
    ...
    if (!newGuru.name || !newGuru.username || !newGuru.password) {
      alert("Mohon lengkapi identitas Nama, Username, dan Kata Sandi.");
      return;
    }
    setIsSubmitting(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex border-b border-slate-200 gap-2">
        <button onClick={() => setMainTab('excel_import')} className={`pb-3 px-4 font-bold text-sm border-b-2 transition cursor-pointer ${mainTab === 'excel_import' ? 'border-sky-600 text-sky-600' : 'border-transparent text-slate-500'}`}>Import Berkas Excel</button>
      ...
      {mainTab === 'excel_import' && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          <button type="button" onClick={onBackToDashboard} className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-sky-600 transition mb-6 group"><ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition" /> Kembali ke Dasbor</button>
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="w-full md:w-64 shrink-0 flex flex-col gap-2">
              <button onClick={() => setUploadType('siswa')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition text-left cursor-pointer ${uploadType === 'siswa' ? 'bg-sky-50 text-sky-700 border border-sky-100' : 'text-slate-600 hover:bg-slate-50'}`}><GraduationCap className="w-4 h-4" /> Data Siswa</button>
              <button onClick={() => setUploadType('guru')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition text-left cursor-pointer ${uploadType === 'guru' ? 'bg-sky-50 text-sky-700 border border-sky-100' : 'text-slate-600 hover:bg-slate-50'}`}><Users className="w-4 h-4" /> Data Guru / Staf</button>
              <button onClick={() => setUploadType('pelanggaran')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-sm transition text-left cursor-pointer ${uploadType === 'pelanggaran' ? 'bg-sky-50 text-sky-700 border border-sky-100' : 'text-slate-600 hover:bg-slate-50'}`}><ShieldAlert className="w-4 h-4" /> Aturan Pelanggaran BK</button>
            </div>
      ...
      {mainTab === 'manage_data' && (
        <div className="space-y-6">
          <div className="flex border-b border-slate-200 bg-white p-2 rounded-xl shadow-sm gap-2">
            <button onClick={() => setActiveSubTab('siswa')} className={`flex items-center gap-2 px-4 py-2 font-bold text-xs rounded-lg transition cursor-pointer ${activeSubTab === 'siswa' ? 'bg-sky-50 text-sky-600' : 'text-slate-500 hover:text-slate-800'}`}><GraduationCap className="w-4 h-4" /> Siswa</button>
            <button onClick={() => setActiveSubTab('guru')} className={`flex items-center gap-2 px-4 py-2 font-bold text-xs rounded-lg transition cursor-pointer ${activeSubTab === 'guru' ? 'bg-sky-50 text-sky-600' : 'text-slate-500 hover:text-slate-800'}`}><Users className="w-4 h-4" /> Guru / Staf</button>
          </div>

          {activeSubTab === 'siswa' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-white p-5 border border-slate-200 rounded-2xl shadow-sm h-fit space-y-4">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2"><PlusCircle className="w-4 h-4 text-sky-600" /> Tambah Siswa Manual</h3>
          ...
          {activeSubTab === 'guru' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-4 bg-slate-50/50 border-b border-slate-100"><h4 className="text-xs font-bold text-slate-700">Daftar Guru Terdaftar ({gurus.length})</h4></div>
                <div className="overflow-y-auto max-h-[520px]">
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-50 text-[11px] font-bold text-slate-400 uppercase tracking-wider sticky top-0 border-b">
                      <tr><th className="p-3 pl-4">Identitas Guru</th><th className="p-3">Mata Pelajaran</th><th className="p-3">Penugasan Peran</th><th className="p-3 text-center">Aksi</th></tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 text-xs text-slate-600">
                      {gurus.map((g: any) => (
      ...
      {/* DIALOG POPUP MODAL UPDATE MASTER */}
      {editingItem && (
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full border shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleUpdatePassword} className="p-5 space-y-4">
              <div>
                <p className="text-xs text-slate-500 font-medium">Memperbarui sandi login untuk:</p>
              ...
              <div>
                ...
                <input type="password" required value={passwordModal.newPass} onChange={(e) => setPasswordModal({ ...passwordModal, newPass: e.target.value })} placeholder="Ketik sandi baru..." className="w-full bg-slate-50 border p-2.5 text-xs font-semibold rounded-xl" />
              </div>
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
